...
Что проверяется для каждого поля:
  ...
  СТАТУС    — 🟢 только при ≥2 разных значениях в окне и хотя бы одном совпадении связи;
              для kind='const' — значение неизменно в окне и есть ≥2 сверки с экраном
...
def main():
    ...
    for fd in F.FIELDS:
        ...
        if kind == 'const':  # поле-константа: подтверждается неизменностью в эфире + сверкой с экраном
            ok_green = distinct == 1 and len(fd.get('screens', [])) >= 2 and not mism
